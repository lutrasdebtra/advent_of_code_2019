    ...
    while len(vaporized) != len(coordinates):
        ...
        for x, y in coordinates:
            if (x, y) not in vaporized:
                dx, dy = x - mx, y - my
                dx, dy = dx // math.gcd(dx, dy), dy // math.gcd(dx, dy)
                closestx, closesty = closest_points.get(
                    (dx, dy), (float("inf"), float("inf"))
                )
                if abs(x - mx) + abs(y - my) < abs(closestx - mx) + abs(closesty - my):
                    closest_points[(dx, dy)] = (x, y)
        vaporized += sorted(
            closest_points.values(), key=lambda p: -math.atan2(p[0] - mx, p[1] - my)
        )
    return vaporized


asteroid_map = open("ten.txt").read().split()
...
vaporization_order = get_vaporization_order(
    asteroid_coordinates, monitoring_p[0], monitoring_p[1]
)
