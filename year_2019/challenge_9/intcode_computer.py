class IntCodeComputer:
    ...
    def _instruction_1(self, modes):
        pos_1, pos_2, pos_3 = (
            ...
            self.instructions[self.pos + 2],
            self.instructions[self.pos + 3],
        )
        self.instructions[
            self._get_output_position(pos_3, modes, 2)
        ] = self._get_instruction(pos_1, modes, 0) + self._get_instruction(
            pos_2, modes, 1
        )
        self.pos += 4
    ...
    def _instruction_3(self, modes):
        if len(self.inputs):
            ...
            self.instructions[
                self._get_output_position(pos_1, modes, 0)
            ] = self.inputs.pop(0)
            self.pos += 2
            return True
        return False
    ...
    def _instruction_7(self, modes):
        ...
        if self._get_instruction(pos_1, modes, 0) < self._get_instruction(
            pos_2, modes, 1
        ):
